...
var routeHelper = module.exports = {
  ...
  routeToPathEntry: function (route, classDef,
    ...
    opts.produces.forEach(current => {
      responseMessages[statusCode].content[current] = {schema: returns};
    });
    ...
    let requestBody = {};
    let formData = [];
    accepts = accepts.filter(current => {
      if (!current.schema || Object.keys(current.schema).length === 0) {
        current.schema = {type: 'string', format: 'JSON'};
      }

      if (current.in === 'formData') {
      ...
      if (current.in === 'body') {
        ...
        opts.produces.forEach(type => {
          requestBody.content[type] = {schema: {$ref: current.schema.$ref}};
        });
        return false;
      }
      return true;
    ...
    if (formData.length) {
      ...
      formData.forEach(current => {
        requestBody.required = requestBody.required || current.required;
        schema.properties[current.name] = {
          type: current.schema.type,
          description: current.description
        };

        if (current.required) {
          schema.required.push(current.name);
        }
      });
    }
    ...
    var entry = {
      ...
      operation: {
        ...
        parameters: accepts.length ? accepts : undefined,
        requestBody: Object.keys(requestBody).length === 0 ? undefined : requestBody,
    ...
    return function (accepts) {
      ...
      if (paramType === 'body') {
        ...
        if (isComplexType) {
          ...
          _assign(paramObject.schema, schema);
        }
      }
